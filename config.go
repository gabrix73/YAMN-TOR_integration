...
// TorConfig contiene la configurazione Tor semplificata
type TorConfig struct {
	Enabled      bool   `yaml:"enabled"`       // Abilita routing Tor
	Required     bool   `yaml:"required"`      // Tor obbligatorio (esce se non disponibile)
	SocksProxy   string `yaml:"socksproxy"`    // Proxy SOCKS5 (solo 127.0.0.1:9050)
	Timeout      int    `yaml:"timeout"`       // Timeout connessione in secondi
	CircuitReset int    `yaml:"circuit_reset"` // Minuti tra reset circuiti automatici
}
...
type Config struct {
	...
	Mail struct {
		...
		ForceTorSMTP    bool `yaml:"force_tor_smtp"`    // Forza SMTP attraverso Tor
		DisableTLSOnion bool `yaml:"disable_tls_onion"` // Disabilita TLS per .onion
	} `yaml:"mail"`
	Stats struct {
	...
	// Configurazione Tor
	Tor *TorConfig `yaml:"tor,omitempty"` // Configurazione Tor (opzionale)
}
...
// initTorConfig inizializza la configurazione Tor con valori di default
func (c *Config) initTorConfig() {
	if c.Tor == nil {
		c.Tor = &TorConfig{
			Enabled:      false,
			Required:     false,
	...
	if c.Tor.SocksProxy == "" {
		c.Tor.SocksProxy = "127.0.0.1:9050"
	}
	if c.Tor.Timeout == 0 {
		c.Tor.Timeout = 30
	}
	if c.Tor.CircuitReset == 0 {
		c.Tor.CircuitReset = 10
	}
}
...
func (f *Flags) newConfig() *Config {
	...
	c.Urls.Fetch = true
	c.Urls.Pubring = "https://echolot.virebent.art/pubring.mix"
	c.Urls.Mlist2 = "https://echolot.virebent.art/mlist2.txt"
	...
	c.Mail.UseTLS = true
	c.Mail.MXRelay = true
	c.Mail.OnionRelay = true // Allow .onion addresses as MX relays
	c.Mail.Sender = ""
	c.Mail.Username = ""
	c.Mail.Password = ""
	...
	c.Mail.ForceTorSMTP = false
	c.Mail.DisableTLSOnion = true
	...
	// Inizializza configurazione Tor con defaults
	c.initTorConfig()
...
func (f *Flags) ParseConfig() (*Config, error) {
	...
	// Assicurati che la configurazione Tor sia inizializzata anche dopo il parsing
	c.initTorConfig()
